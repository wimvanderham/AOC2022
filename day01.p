...
DEFINE VARIABLE lOpenURL     AS LOGICAL   NO-UNDO.
...
/* ***************************  Main Block  *************************** */
DISPLAY
   lOpenURL LABEL "Open URL?"     COLON 15 SKIP 
...
COPY-LOB FROM FILE "2022\input\01.txt" TO OBJECT lcInput.
...
IF lPart[1] THEN DO:
   ...
   VIEW-AS ALERT-BOX TITLE " 2021 - Day 12 - Part One".
...
IF lPart[2] THEN DO:
   ...
   MESSAGE 
      SUBSTITUTE ("Solution: &1.", 
         iSolution) SKIP (1)
      SUBSTITUTE ("Found solution in &1 msecs.", ETIME)
   VIEW-AS ALERT-BOX TITLE " 2021 - Day 12 - Part Two".
