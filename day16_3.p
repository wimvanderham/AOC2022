...
DEFINE TEMP-TABLE ttVisited
   ...
   FIELD iMaxValue     AS INTEGER   FORMAT "zzz9"  LABEL "Max Value"
...
IF lPart[2] THEN DO:
   ...
   DO iOpenValves = 0 TO INTEGER (iMaxValveStates / 2) - 1:
      ...
      iSolution = MAXIMUM (iSolution, 
                           getMaxFlow("AA", 26, getOpenList(cLeftSwitches), 1) + 
                           getMaxFlow("AA", 26, getOpenList(cRightSwitches), 1)).
      IF lvlShow THEN DO:
         PUT UNFORMATTED 
         SUBSTITUTE (" Solution = &1", iSolution) SKIP.
      END.                            
   ...
   */
   ...
   /* Search for the best path from start valve AA 
   ** with 26 minutes of time left and 
   ** no open valves
   ** and 2 player */
   IF lvlShow THEN DO:
      OUTPUT TO "output\16_2b.txt".
      PUT UNFORMATTED 
      SUBSTITUTE ("Run of &1.", STRING (NOW, "99-99-9999 HH:MM:SS")) SKIP.
   END.

   iSolution = getMaxFlow("AA", 26, "").
   
   IF lvlShow THEN DO:
      PUT UNFORMATTED 
         SUBSTITUTE ("First Player: &1 Open List: &2", iSolution, lvcOpenList) SKIP.
      OUTPUT CLOSE.
   END.
   
   MESSAGE "Part Two - First player got:" iSolution SKIP 
   "Open List:" lvcOpenList
   VIEW-AS ALERT-BOX.
   
   IF lvlShow THEN DO:
      OUTPUT TO "output\16_2b.txt" APPEND.
   END.
   
   iSolution = iSolution + getMaxFlow("AA", 26, lvcOpenList).
   
   IF lvlShow THEN DO:
      PUT UNFORMATTED
         SUBSTITUTE ("Second Player: &1 Open List: &2", iSolution, lvcOpenList) SKIP.
      OUTPUT CLOSE.
   END.
...
DEFINE VARIABLE iPrevMaxValue AS INTEGER   NO-UNDO.
   ...
   */
   ...
   WHERE ttNextValve.cValve EQ ttPath.cToValve:
      /* All reachable paths from this position */
      IF lvlShow THEN
         PUT UNFORMATTED 
         SUBSTITUTE ("to Valve: &1, Path: &2, Flow rate: &3.",
                     ttNextValve.cValve,
                     ttPath.iMinutes,
                     ttNextValve.iFlowRate) SKIP.
      ...
      iPrevMaxValue = iMaxValue.
      iMaxValue = MAXIMUM (iMaxValue, getMaxFlow(ttNextValve.cValve, iTimeLeft, cNewOpenList) + (iTimeLeft * ttNextValve.iFlowRate)).
      IF iMaxValue GT iPrevMaxValue THEN DO: 
         lvcOpenList = cNewOpenList.
         IF NUM-ENTRIES (cNewOpenList) LT NUM-ENTRIES (lvcOpenList) THEN 
            lvcOpenList = cNewOpenList.
      END.
         
         
      IF lvlShow THEN DO:
         IF ipcValve EQ "AA" THEN 
            PUT UNFORMATTED SKIP (1)
               SUBSTITUTE ("*** From &1 --> &2, found Max Value: &3 with Open List: &4",
                           ttValve.cValve,
                           ttPath.cToValve,
                           iMaxValue,
                           lvcOpenList) SKIP(2).
   ...
   END. /* All reachable paths from this position */
